...
2. 确认窗口右上角显示当前版本，例如 **Version 0.8.1 (81)**。
3. 点击 **Choose Eagle Library...**，选择你的 Eagle `.library` 文件夹。
3. 第一次选择后，App 会自动构建索引并提示 **Building index. Please wait...**。请等它完成。
4. 索引完成后，底部状态会显示索引数量、视频数量和缓存路径，例如 `Index has ... items (... videos). Cache: ...`。
5. 点击 **Settings** 打开 macOS **系统设置 -> 屏幕保护程序**，选择 **Eagle Grid Saver**。
6. 在系统屏保的 **Options...** 面板右上角确认版本号，确保系统加载的是新版 `.saver`。
7. 需要立即查看效果时，回到 App 点击 **Start Screen Saver**。
8. 如果系统屏保里没有素材，点击该屏保的 **Options...**，在里面重新选择 Eagle `.library`。这一步能让真正的系统屏保宿主获得文件读取权限。
9. 后续往 Eagle 里新增或删除素材后，打开 App 点击 **Update Index** 即可更新索引。
...
2. Check the version in the top-right corner, such as **Version 0.8.1 (81)**.
...
5. When the index is ready, the bottom status shows the item count, video count, and cache path, such as `Index has ... items (... videos). Cache: ...`.
6. Click **Settings** to open macOS **System Settings -> Screen Saver**, then choose **Eagle Grid Saver**.
7. In the screen saver **Options...** panel, check the version in the top-right corner to confirm macOS loaded the latest `.saver`.
8. To see it immediately, return to the app and click **Start Screen Saver**.
9. If the full system screen saver cannot read your library, open **Options...** for the screen saver and choose the Eagle `.library` there. This gives the actual macOS screen saver host permission to read the folder.
10. After adding or deleting Eagle assets later, open the app and click **Update Index**.
