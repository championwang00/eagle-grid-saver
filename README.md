...
从 `0.9.3` 开始，安装包会自动删除当前登录用户目录里的旧版 `~/Applications/Eagle Grid Saver.app` 和 `~/Library/Screen Savers/EagleGridSaver.saver`，并刷新屏保宿主缓存。安装后请从 `/Applications/Eagle Grid Saver.app` 打开新版 App。
...
2. 确认窗口右上角显示当前版本，例如 **Version 0.9.8 (98)**。
3. 点击 **Choose Eagle Library...**，选择你的 Eagle `.library` 文件夹。
3. 第一次选择后，App 会自动构建索引并提示 **Building index. Please wait...**。请等它完成。
4. 使用 **Scroll Speed** 滑块调节滚动速度，默认是 `1.0x`，范围是 `0.25x` 到 `10.0x`；使用 **Columns** 选择横向列数，范围是 `1` 到 `6` 列，默认 `2` 列。这些设置会立即写入屏保运行配置，不需要重新构建索引。手动重建索引时，App 也会自动刷新屏保宿主的旧缓存和旧路径配置。
5. 索引完成后，底部状态会显示索引数量、视频数量和缓存路径，例如 `Index has ... items (... videos). Cache: ...`。
6. 点击 **Settings** 打开 macOS **系统设置 -> 屏幕保护程序**，选择 **Eagle Grid Saver**。
7. 在系统屏保的 **Options...** 面板右上角确认版本号，确保系统加载的是新版 `.saver`；这里也可以调节 **Scroll Speed** 和 **Columns**。
8. 这一步必须手动做一次：在系统弹窗里把 **使用屏幕保护程序** 设为 **自定义**，并点击选中 **Eagle Grid Saver**。如果跳过这一步，macOS 可能继续播放太浩湖、红杉等系统屏保。
9. 需要立即查看效果时，回到 App 点击 **Start Screen Saver**。如果 App 检测到系统选择还不可靠，会再次打开系统设置并提示手动选择。
10. 如果系统屏保里没有素材，点击该屏保的 **Options...**，在里面重新选择 Eagle `.library`。这一步能让真正的系统屏保宿主获得文件读取权限。
11. 后续往 Eagle 里新增或删除素材后，打开 App 点击 **Update Index** 即可更新索引。
...
Starting with `0.9.3`, the installer removes stale per-user copies at `~/Applications/Eagle Grid Saver.app` and `~/Library/Screen Savers/EagleGridSaver.saver`, then refreshes screen saver host caches. After installing, open the new app from `/Applications/Eagle Grid Saver.app`.
...
2. Check the version in the top-right corner, such as **Version 0.9.8 (98)**.
