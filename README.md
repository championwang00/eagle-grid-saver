...
2. 确认窗口右上角显示当前版本，例如 **Version 0.9.3 (93)**。
3. 点击 **Choose Eagle Library...**，选择你的 Eagle `.library` 文件夹。
3. 第一次选择后，App 会自动构建索引并提示 **Building index. Please wait...**。请等它完成。
4. 使用 **Scroll Speed** 滑块调节滚动速度，默认是 `1.0x`，范围是 `0.25x` 到 `10.0x`。速度设置会立即写入屏保运行配置，不需要重新构建索引。手动重建索引时，App 也会自动刷新屏保宿主的旧缓存和旧路径配置。
...
2. Check the version in the top-right corner, such as **Version 0.9.3 (93)**.
